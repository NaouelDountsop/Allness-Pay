...
export function AddLinkedAccountModal({ open, onOpenChange, wallets }: AddLinkedAccountModalProps) {
  ...
  const isStep2Valid = () => {
    ...
    if (accountType === "international_account") return accountNumber.length > 0 && bankName.length > 0;
    return false;
  };
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        {step === 1 && (
          <div className="space-y-3">
            <p className="text-sm text-gray-600">Sélectionnez le type de compte que vous souhaitez lier</p>
            ...
            <div className="flex items-start gap-2 bg-afrilink-green/5 border border-afrilink-green/20 rounded-xl p-3 mt-4">
              ...
              <p className="text-xs text-gray-600">
                Vos informations sont sécurisées et cryptées. Elles ne sont jamais partagées sans votre consentement.
              </p>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-3 bg-gray-50 p-4 rounded-xl text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">
                {wallets.find((w) => w.id === selectedWalletId)?.label ?? wallets.find((w) => w.id === selectedWalletId)?.walletNumber}
              </span>
            </div>
            <div className="flex justify-between">
