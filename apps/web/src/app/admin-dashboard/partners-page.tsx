...
function ServiceTag({ label }: { label: string }) {
  return (
    <span className="rounded-full bg-gray-50 px-2 py-0.5 text-[10px] font-medium text-gray-600">{label}</span>
  );
}
...
function PartnerRow({ partner, onSelect }: { partner: Partner; onSelect: () => void }) {
  return (
    <tr className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
      ...
      <td className="whitespace-nowrap px-3 py-2">
        <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${STATUS_STYLES[partner.status]}`}>
          {partner.status}
        </span>
      </td>
      <td className="whitespace-nowrap px-3 py-2 text-xs font-medium text-gray-900">{partner.volume}</td>
      ...
      <td className="whitespace-nowrap px-3 py-2">
        <div className="flex items-center gap-1">
          ...
          <button className="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-600" aria-label="Plus d'options">
    ...
    <div className="rounded-xl border border-gray-100 bg-white p-3 shadow-sm">
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        <div>
          ...
          <span className={`mt-0.5 inline-block rounded-full px-1.5 py-0.5 text-[9px] font-semibold ${STATUS_STYLES[partner.status]}`}>
            {partner.status}
          </span>
        </div>
      </div>
      ...
      <Button variant="outline" size="sm" className="mt-2 w-full justify-center rounded-lg text-[11px]">
              ...
              <div className="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 px-3 py-2">
                  <p className="text-[10px] text-gray-500">
                    Affichage de {(page - 1) * pageSize + 1} à {Math.min(page * pageSize, totalPartners)} sur{" "}
